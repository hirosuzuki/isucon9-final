...
func trainSearchHandler(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	if fromStation, ok = stationMap[fromName]; !ok {
		log.Print("fromStation: no rows")
		errorResponse(w, http.StatusBadRequest, err.Error())
		return
	}

	if toStation, ok = stationMap[toName]; !ok {
	...
	if trainClass == "" {
		query := "SELECT * FROM train_master WHERE date=? AND train_class IN (?) AND is_nobori=? ORDER BY date, departure_at"
		inQuery, inArgs, err = sqlx.In(query, date.Format("2006/01/02"), usableTrainClassList, isNobori)
	} else {
		query := "SELECT * FROM train_master WHERE date=? AND train_class IN (?) AND is_nobori=? AND train_class=? ORDER BY date, departure_at"
	...
	for _, train := range trainList {
		...
		if isContainsOriginStation && isContainsDestStation {
			...
			err = dbx.Get(&departure, "SELECT departure FROM train_timetable_master WHERE date=? AND train_class=? AND train_name=? AND station=?  ORDER BY train_name, departure", date.Format("2006/01/02"), train.TrainClass, train.TrainName, fromStation.Name)
			if err != nil {
			...
			premium_avail_seats, premium_smoke_avail_seats, reserved_avail_seats, reserved_smoke_avail_seats, err := train.getAvailableSeats4(fromStation, toStation)
			if err != nil {
				errorResponse(w, http.StatusBadRequest, err.Error())
				return
			}
...
func trainSeatsHandler(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	// 対象列車の取得
	var train Train
	query := "SELECT * FROM train_master WHERE date=? AND train_class=? AND train_name=? ORDER BY date, departure_at"
...
func trainReservationHandler(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	if departureStation, ok = stationMap[tmas.StartStation]; !ok {
		tx.Rollback()
	...
	// Arrive
	if arrivalStation, ok = stationMap[tmas.LastStation]; !ok {
		tx.Rollback()
	...
	// From
	if fromStation, ok = stationMap[req.Departure]; !ok {
		tx.Rollback()
	...
	// To
	if toStation, ok = stationMap[req.Arrival]; !ok {
		tx.Rollback()
